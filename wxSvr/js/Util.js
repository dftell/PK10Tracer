...
wx.ready(function () {
    ...
    document.querySelector('#chooseImage').onclick = function () {
        wx.chooseImage({
            success: function (res) {
                ...
                LoadLocalImage(res.localIds[0]);
                //alert('已选择 ' + res.localIds.length + ' 张图片');
            }
        });
    };

    function BaiDu_ocr(img, isPos) {

        var imag64 = img;// images.toBase64(img, "png", 100);
        ...
        var getTokenUrl = "https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=IMi7uTlPbISgrYCkBnUZxREn&client_secret=NRE9cT0SA9qeEyadk7e0wzHH2LHiQTeS";
        //token获取地址。
        var token_Res;
        try {
            var data = {
            ...
            zepto.get(getTokenUrl, data,
                function (t) {
                    alert(t);
                    getImgString(t, img, isPos);
                },
                function () {
                    alertTip("token连接错误","获取token");
                }
            );
        }
        catch (ce) {
    ...
    function LoadLocalImage(localId)
    {
        img64 = null;
        if (window.__wxjs_is_wkwebview)
        {
            wx.getLocalImgData({ //循环调用 getLocalImgData 
                ...
                success: function (res) {
                    ...
                    rows = "";
                    document.querySelector('#chooseImage').src = localId;
                },
        ...
        {
            document.querySelector('#chooseImage').src = localId;
            document.querySelector('#chooseImage')Text = localId;
        }
    }

    function getImgString(token_Res, imag64, isPos) {
        //alert(obj2jsonString(token_Res));
        ...
        var ocr_Res = wx.request(ocrUrl, data,
            function (t) {
                valpwd = t.words_result[0].words;
                alert(JSON.stringify(t));

                //alert(t.words_result[0].words);
            }
            , function (t) {
                //alertTip("获取验证码文本","获取文本连接错误");
            }
        );
