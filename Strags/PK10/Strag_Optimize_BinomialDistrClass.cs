...
{
    [DescriptionAttribute("二项分布优化选号策略"),
        DisplayName("二项分布优化选号策略")]
    [Serializable]
    ...
    { 
        ...
        void CalcPeakValues(int N, int K,double p)
        {
            ...
            if (AllBinomPeaks.ContainsKey(key))//静态的，为什么存在了对应关键字就不再增加了？后面的数据完全不采纳？
            {
                return;
            }
        ...
        void InitAllPeaks()
        {
            ...
            {
                CalcPeakValues(i, this.InputMaxTimes, 0.098);//为什么是0.098？this.InputMaxTimes=14
            }
        }
