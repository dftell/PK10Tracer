...
namespace StrategyLibForWD
{
    public class GLClassProcess
    {
        ...
        public string UpdateAllGLList(DateTime EndT)
        {
            ...
            dt = wss.getRecods(w,strSecCodes, "windcode,sec_name,launchdate");
            if (dt == null || dt.Rows.Count == 0)
            {
                jgdb.Close();
                return "无法从万得数据库中获取到概念其它信息！";
            }
            DataTable InsertTable = dbdt.Clone();
        ...
        public DataTable getRecords(string strCode, DateTime dt)
        {
            ...
            {
                wsetobj = new WSETMarketClass(w,globj.GLCode, dt);
            }
            else//如果是指数类
            {
                wsetobj = new WSETIndexClass(w,globj.GLCode, dt);
            }
